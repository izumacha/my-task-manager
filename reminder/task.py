...
# 遡って当日に含める設計（タスクは高々 2 日にまたがる前提）。timeline.carry_over_overdue の
# 繰り越しスキップ判定（end_dt > day_start）も同じ前提に依存しており、MAX_DURATION を 24*60 より
# 大きくすると「2 日以上前のタスクが繰り越されず表示もされない」空白が生じ得るため、
# 変更時は timeline.py 側の両判定を合わせて見直すこと。
MIN_DURATION = 5
MAX_DURATION = 24 * 60
DEFAULT_DURATION = 30
