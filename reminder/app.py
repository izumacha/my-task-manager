...
class PlannerApp:
    ...
    def _refresh_stale_start_default(self) -> None:
        ...
        既知の制限: 「未変更」判定は表示値と前回の自動補完値の文字列一致でしか
        判定できない。ユーザーが手で入力した値がたまたま自動補完値と同じ
        （例: 起動直後の既定値をそのまま打ち直した）場合は見分けが付かず、
        後で現在時刻基準の値に上書きされ得る。実害は数分程度のズレに留まり
        （本メソッドが防ぐ翌日への繰り上げほど深刻ではない）、正しく見分ける
        には StringVar への trace コールバックでの変更検知が必要になり、
        この修正の範囲を超えるため見送っている。
        """
    ...
    def _draw_task_block(self, cv: tk.Canvas, row: TimelineRow,
        ...
        # レーンが狭い（同時刻に多数のタスクが重なる）と、この固定オフセットが
        # 自分のブロック幅を超えて隣のレーンまではみ出してしまう。すると描画上
        # 隣のタスクの上に重なって見えるだけでなく、クリック判定用の cb_box も
        # 隣のレーンへ流出し、そのタスクの完了操作が別タスクに誤爆する（レーン幅の
        ...
        # テキストの開始 x 座標（本来はチェックボックスの右側）。cb_cx をブロック右端
        # (x1) 側でクランプしても、そこからさらに + r + 8 したこの位置は依然として
