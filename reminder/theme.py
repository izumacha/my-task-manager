...
# ウィンドウタイトル・ダイアログタイトル・デスクトップ通知タイトルなど、
# アプリ名を表示する箇所すべてが参照する唯一の文字列（CLAUDE.md §6「定数・
# ラベルは一元管理する」。表示名を変えるときはここだけ編集すればよい）。
APP_NAME = "my-task-manager"  # アプリケーション名（UI 各所のタイトル表示に使う）
...
# タスクカード（_draw_task_block）・時刻ラベル（_draw_time_grid）の内側で使う
# 細かな余白トークン。以前は app.py に生の数値として直書きされていたため、
# CLAUDE.md §6「見た目の値をコードに直書きしない」に従いここへ集約する。
CAL_LABEL_GAP = 8          # 罫線（CAL_GUTTER）と時刻ラベル文字の間の隙間（ピクセル単位）
CAL_STRIPE_INSET_X = 3     # カテゴリ色ストライプの左右内側マージン（ピクセル単位）
CAL_STRIPE_INSET_Y = 4     # カテゴリ色ストライプの上下内側マージン（ピクセル単位）
CAL_CHECK_HIT_PAD = 3      # チェックボックスの円半径に足すクリック判定領域の余白（ピクセル単位）
CAL_CHECK_OFFSET_X = 16    # ストライプ右隣に置くチェックボックス中心の基準オフセット（ピクセル単位）
CAL_CHECK_OFFSET_Y = 16    # 高いカードでチェックボックスを上寄せする際の上端からのオフセット（ピクセル単位）
CAL_TEXT_GAP = 8           # チェックボックスとテキストの間、およびテキストとカード右端の隙間（ピクセル単位）
CAL_TEXT_MIN_WIDTH = 10    # テキストの折り返し幅の最低値（ピクセル単位）
