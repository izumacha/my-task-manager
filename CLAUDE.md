...
`my-task-manager` は Python + tkinter 製の GUI タスクプランナーアプリ（`reminder/` パッケージ）のリポジトリです。Any Planner を参考に、1 日のタスクを**カレンダー（デイビュー）**で可視化し、空き時間や「あとでやる」リストを扱います。繰り返しタスクは「完了した時点」を起点に日/週/月/年で再スケジュールされます。
