...
- **機能**
  - **今日のカレンダー（デイビュー）**: 起床〜就寝を縦の時間軸に取り、タスクを
    **所要時間ぶんの高さを持つ色付きブロック**として配置します。1 時間ごとの罫線・
    時刻ラベル・現在時刻ライン（now ライン）付きで、**空き時間は「ブロックの無い余白」**
    としてそのまま見えます。ブロックはクリックで選択でき、状態で色が変わります
    （完了＝✓グレー、進行中＝ティール塗り、過去未了＝赤、これから＝カテゴリ色）
